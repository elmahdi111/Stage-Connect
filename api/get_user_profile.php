...
try {
    $stmt = $pdo->prepare('SELECT id, name, email, bio, profile_pic FROM users WHERE id = ?');
    ...
    // Get user's posts
    $pstmt = $pdo->prepare('SELECT id, user_id, content, image_path, created_at FROM posts WHERE user_id = ? ORDER BY created_at DESC LIMIT 50');
