...
// If a logged-in trainee submits, associate their user_id with the application
$userId = null;
if (isset($_SESSION['user']) && isset($_SESSION['user']['id']) && isset($_SESSION['user']['type']) && $_SESSION['user']['type'] === 'trainee') {
...
try {
    ...
    $status = 'pending';
    try {
        // Try inserting with resume_path (if the column exists). If user id available, include it.
        if ($userId !== null) {
            $stmt = $pdo->prepare('INSERT INTO applications (user_id, internship_id, applicant_name, email, phone, school, date_applied, status, extra, resume_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
            $stmt->execute([$userId, $internship_id, $name, $email, $phone, $school, $date, $status, $extra, $resumePath]);
        } else {
        ...
        try {
            if ($userId !== null) {
                $stmt = $pdo->prepare('INSERT INTO applications (user_id, internship_id, applicant_name, email, phone, school, date_applied, status, extra) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
                $stmt->execute([$userId, $internship_id, $name, $email, $phone, $school, $date, $status, $extra]);
            } else {
